...
| `src/main/services/cold-storage/index.ts` | `createColdStorageService` — storage-mode path helpers (`getDownloadDir`, `getColdStorageDir`, `isAdvanced`, `dirsForScan`) + `scanUsage` (hot/cold bucket walk → `StorageUsage`). Move/delete/cleanup ops still live in `index.ts` (deferred to a follow-up — disk-mutation surface kept isolated for separate review). |
