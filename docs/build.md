...
The macOS packaging step flakes on `security: SecKeychainUnlock: The user name or passphrase you entered is not correct`, thrown from inside electron-builder's own temporary-keychain handling. It is transient, not a secrets problem — the same secrets succeed minutes later — so the **"Package (mac, up to 2 attempts)"** step in `release.yml` retries once. Retrying alone is not enough, and the reset in that step is load-bearing: electron-builder removes its temporary keychain only on the success path, and because the keychain's name is derived from the signing cert the next attempt resolves to that same file and fails to unlock it with the password it generated — the identical `SecKeychainUnlock` error. So the step deletes the orphaned keychain and restores the keychain search list before retrying. On a forced-failure probe, attempt 2 failed deterministically without the reset and succeeded with it. The cap is 2: a genuine cert or secret failure still fails both. The other platforms use a separate single-attempt step so a real build break fails on the first try, and `strategy.fail-fast: false` keeps a mac flake from cancelling the healthy linux/win legs.
