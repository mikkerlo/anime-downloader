...
| `player:remux-mkv-stream` | invoke | ffprobe MKV + start fragmented-MP4 pipe from ffmpeg; returns an `MseOpenResult` for MSE consumption — `{ sessionId, generation, duration, mimeType, hasSubtitlesPending, contentStart }` — declared once in `src/shared/types/player.d.ts` (#294) and referenced by name from main and preload; the renderer never names it, it reads the reply structurally off the `window.api` signature. `contentStart` is the run's measured `sourceBuffer.timestampOffset` (see `docs/player.md`); the `initialSeek` *argument* is the request, and is bounded against the probed duration (#275). Short-circuits to `{ requiresTranscode: true }` — before spawning anything — when the file is HEVC and `hevcTranscodeOnPlay` is `always` |
