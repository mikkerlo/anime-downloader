...
| `player:remux-mkv-stream` | invoke | ffprobe MKV + start fragmented-MP4 pipe from ffmpeg; returns an `MseOpenResult` for MSE consumption — `{ sessionId, generation, duration, mimeType, hasSubtitlesPending, contentStart, refusedSeek }` — declared once in `src/shared/types/player.d.ts` (#294) and referenced by name from main and preload; the renderer never names it, it reads the reply structurally off the `window.api` signature. `contentStart` is the run's measured `sourceBuffer.timestampOffset` (see `docs/player.md`); the `initialSeek` *argument* is the request, and is bounded against the probed duration (#275); `refusedSeek` reports whether that bound rejected it, so the renderer drops its resume land on main's decision instead of re-deriving it (#295). Short-circuits to `{ requiresTranscode: true }` — before spawning anything — when the file is HEVC and `hevcTranscodeOnPlay` is `always` |
| `player:remux-mkv-stream-transcode` | invoke | Same as `player:remux-mkv-stream` (same `MseOpenResult` reply, minus the `{ requiresTranscode: true }` arm) but re-encodes video to H.264 (and audio to AAC when not already AAC) for platforms with no HEVC decoder |
...
| `player:close-stream-session` | invoke | Reap **one** stream session by id — SIGKILL its ffmpeg, unlink its `${baseName}-${sessionId}.ass`, drop it from the map. The targeted counterpart to the blanket `player:cleanup-remux`, used by a superseded `prepareMkvForPlayback` to close the session its own open registered without touching a concurrent winner's session or the shared tmpDir (#291). Guarded on `session.senderId === event.sender.id` — a cross-`webContents` close is refused with `{ closed: false }`, never a throw. Does **not** bump `cleanupGeneration`: the renderer only learns a `sessionId` from an open's reply, so it can never race a parked open handler |
...
| `syncplay:set-file` | invoke | Announce the currently-playing file to the room as `{canonicalName, duration, features.animeDlAppMeta:{animeId, malId, episodeInt, translationId}}`. The payload also carries an optional `newPlayer` (#236), true on the first push of each `useSyncplayClient` mount — main keys `setFile()`'s adoption and readiness resets on it — and an optional `playerSessionId` (#307), constant for the life of a mount and different between mounts, which main stores beside `currentFile` so a later `syncplay:player-closed` can prove the close belongs to the player that announced it. Both are stripped before the wire: `sendSetFile()` builds the outbound `Set: {file}` field by field, so a renderer→main field cannot leak into the Syncplay protocol |
...
| `syncplay:player-closed` | invoke | The player was torn down — sent from `useSyncplayClient`'s `onBeforeUnmount` (#288). Two effects, and only one of them is gated. Main clears `lastSnapshotAt`, `playbackAdopted` and `seekIntent` **unconditionally**, with no timestamp compare and no arrival-order test, because `PlayerView` is the only mount site of the composable and is `v-if`-gated with no `key` and no `<KeepAlive>` — so a close always unmounts before a reopen mounts, and the two ride one renderer's `invoke` queue in that order. Without it main infers the close from silence on the 5 s `PLAYBACK_STALE_MS` horizon and keeps asserting the closed player's frozen position into the room for that whole window. **The payload (#307)** is the optional `playerSessionId` of the mount that is closing — the same value that mount announced its file under — and it gates only the second effect: nulling `currentFile` + the stored ID and sending `Set: {file: null}` to retire our seat in `Room.getPosition()`'s `min()`. The channel was payload-free before #307 precisely because the first effect needs no guard; the second one does, and the asymmetry is that the reset is self-repairing in one snapshot (`isAdopted()`'s drift latch) while nothing re-announces a file — `sendSetFile()` has exactly two callers, `setFile()` and `finishHandshake()`. An omitted or non-matching ID (an un-plumbed renderer, or a mount that skipped its push at the readiness guard and so is quoting an ID main never stored) is unmatchable rather than a wildcard, so that close keeps head's behaviour exactly. A matching close with no socket still clears locally, which is what stops the reconnect re-announce re-advertising a dead player's file. See [Watch Together](./syncplay.md) |
...
| `syncplay:get-status` | invoke | Return the current `{state, host, port, room, username, tls, error?}` for initial hydration, plus the three projected fields below |
...
| `syncplay:connection-status` | send | State machine transitions (`idle` → `connecting` → `hello-sent` → `ready` → `reconnecting`/`disconnected`), plus the three projected fields below |
...
**Three projected `SyncplayStatus` fields (#228, #281).** `playbackAdopted` (main's adoption latch — "our playback has converged, so what we assert now reaches the wire"), `roomPaused` (`lastRoomState.paused`, recorded above `handleState()`'s echo guards) and, since #281 slice B, `outOfFile` (*which* de-adoption this is: the room's projected position lies past the end of the file we announced, so `playbackAdopted: false` alone cannot tell "converging" from "the room is somewhere I cannot go") ride the existing `syncplay:connection-status` broadcast and `syncplay:get-status` reply; no new channel, and the preload is an unchanged pass-through. They are **overlaid at emit/read time, never stored on the status object**: `tearDown()` clears the underlying values without emitting a status, so a mirrored copy would leak session 1's `true` into session 2's whole pre-adoption window. `outOfFile` is recomputed from `lastRoomState` + `currentFile.duration` on every read for the same reason, rather than storing the boolean `handleState()`'s de-adoption predicate decided on. Because none of the underlying values changes on a status *transition*, `emitStatusIfProjectionChanged()` re-emits the current status (an empty `setStatus({})` patch, so `state`/`error` are preserved) when a projection differs from the last emitted one — from the heartbeat tick after `sendStateMessage()` and from `handleState()` after the `lastRoomState` write. At most one extra emit per real flip. The renderer's pending-pause hold reads the first two, and its out-of-file pause rule reads the third (see [Watch Together](./syncplay.md)).
