...
| `syncplay:player-closed` | invoke | The player was torn down — sent from `useSyncplayClient`'s `onBeforeUnmount` (#288). **No payload**: main clears `lastSnapshotAt`, `playbackAdopted` and `seekIntent` unconditionally, with no timestamp compare and no arrival-order test, because `PlayerView` is the only mount site of the composable and is `v-if`-gated with no `key` and no `<KeepAlive>` — so a close always unmounts before a reopen mounts, and the two ride one renderer's `invoke` queue in that order. Without it main infers the close from silence on the 5 s `PLAYBACK_STALE_MS` horizon and keeps asserting the closed player's frozen position into the room for that whole window. See [Watch Together](./syncplay.md) |
