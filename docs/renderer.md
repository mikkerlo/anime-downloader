...
- **Restyled screens** — Home (#162), Search + Library (#163), Calendar (#164), and the Anime Detail centerpiece (#165: hero backdrop + fixed 196px `.detail-poster`, `.detail-cols` `1fr/304px` two-column layout collapsing under 1100px, restyled `EpisodeList`/`EpisodeRow` with poster thumbnails + mono episode numbers, and the `ShikimoriPanel` `.status-seg`/`.score-pick` pickers, `FriendsPanel`, `ChronologyPanel`, `SkipDetectionPanel` side/full-width panels). Each consumes the tokens + shared primitives above and keeps screen-specific classes (`.detail-*`, `.ep-*`, `.cal-*`, `.side-panel`, `.skip-*`) scoped.
