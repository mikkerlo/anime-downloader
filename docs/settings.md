...
| `syncplayPassword` | string | `''` | Syncplay server password, stored as-is like `token` / `shikimoriCredentials`. Deliberately its own key rather than a field on `syncplay` — the renderer overwrites that object wholesale on every settings save and would drop it. Written by `syncplay:set-password` and never sent back to the renderer |
...
| `priorityAnimeIds` | array | `[]` | Smotret-anime IDs (as strings) the user flagged as "planned with high priority", in promote order, backing the Library view's Priority tab (#269). IDs only — the metadata home stays `library`, so a prioritized title always has a `library` entry (`library-set-priority` stars on promote, `library-toggle` demotes on un-star). Purely local: no Shikimori dependency, untouched by rate refreshes, session expiry and `shikimori:logout`, so it works signed out. The array (rather than a `Record`) fixes display order on disk so drag-to-reorder needs no migration later |
