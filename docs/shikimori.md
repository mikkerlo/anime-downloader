...
- **Taste profile** (`buildTasteProfile`) — folds the cached `shikimoriUserRates` into a per-genre affinity map. A title's sentiment is positive when completed/scored-high, **negative when dropped** (a stronger signal than a merely low score), so a dropped title's genres actively suppress similar candidates. Genres come from the cached `shikimoriAnimeDetails` payloads (the same prefetch cache the profile dashboard uses); titles without cached details still contribute to the rated-set exclusion. `ratedMalIds` (every status) is the exclusion set — recommendations never include a title already on the user's list.
- **Candidate generation** — seeds are the user's top-8 scored `completed`/`watching`/`rewatching` titles. For each seed the handler calls `getSimilar(token, malId)` sequentially (≤8 calls/build, under Shikimori's 5 req/s cap and reused from cache). Results are merged into a candidate pool keyed by MAL ID, tracking per-candidate seed provenance + consensus (how many seeds surfaced it).
