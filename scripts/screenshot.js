...
function ocr(imagePath) {
  ...
  try {
    execSync(`convert "${imagePath}" -gravity South -crop 100%x15%+0+40 "${cropPath}"`, {
      stdio: 'pipe'
    })
...
;(async () => {
  ...
  if (!fs.existsSync(mkvTestFile)) {
    try {
      execSync(
        'ffmpeg -f lavfi -i color=c=0x1a1a2e:s=640x480:d=40:r=24 -f lavfi -i sine=frequency=440:duration=40 -c:v libx264 -preset ultrafast -c:a aac /tmp/test-h264.mkv -y',
        { stdio: 'pipe' }
      )
    } catch {
      /* may already exist */
  ...
  if (fs.existsSync(mkvTestFile)) {
    ...
    async function seekAndScreenshot(seekTime, label, screenshotPath, expectedText) {
      ...
      if (expectedText) {
        ...
        if (found) {
          ...
          console.log(
            `⚠️  OCR did NOT find expected text "${expectedText.join('" or "')}" in screenshot`
          )
        }
      }
    }
    ...
    await seekAndScreenshot(2, '"Just get up and come."', '/tmp/app-ass-dialogue.png', [
      'get',
      'up',
      'come'
    ])
    ...
    await seekAndScreenshot(
      15,
      'sign translations (Health Center / Yu Yu Land)',
      '/tmp/app-ass-signs.png',
      ['Health', 'Yu', 'Land', 'Center']
    )

    // Screenshot 4: "Nobody's around now..." — subtitle at 0:20–0:26, seek to 21s
    await seekAndScreenshot(21, '"Nobody\'s around now..."', '/tmp/app-ass-nobody.png', [
      'Nobody',
      'around',
      'ourselves',
      'place'
    ...
    await seekAndScreenshot(28, '"This feels really good!"', '/tmp/app-ass-good.png', [
      'feels',
      'good',
      'really'
    ])
