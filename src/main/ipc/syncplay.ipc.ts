...
export function register({ store }: AppDeps): void {
  ...
  // `SyncplayStatus` — connection state (stored in main as `SyncplayConnectionStatus`)
  // plus the projections — is what the lifetime-scoped `useSyncplayStore` caches for UI
  // hydration, and a per-open playhead read served from that cache would be stale.
