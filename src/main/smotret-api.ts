...
export class SmotretApi {
  ...
  private async request(path: string): Promise<unknown> {
    ...
    // so other server-side codes are not missed (#354). The inner `if (err)`
    // is load-bearing: `{"error":null,"data":{…}}` is a success and must not
    // become a throw — don't collapse it into `err?.code ?? 0`.
