...
export class SyncplayClient extends EventEmitter {
  ...
  private handleState(payload: unknown): void {
    ...
    // (src/renderer/src/composables/use-syncplay-client.ts:1676-1677) and owns the pending-pause release and
    ...
    // (src/renderer/src/composables/use-syncplay-client.ts:1316), so a paused scrub lands every peer up to
    ...
    // element (src/renderer/src/composables/use-syncplay-client.ts:795-797, SNAPSHOT_MIN_INTERVAL_MS at
    ...
    // move nobody made (src/renderer/src/composables/use-syncplay-client.ts:1656-1658) and a "Paused by me"
    // badge for a pause we did not press (`syncplayPausedBy`, src/renderer/src/composables/use-syncplay-client.ts:1256-1257). `null`
