...
// `src/renderer/src/composables/use-syncplay-client.ts:2162` into whatever runs next. The wrapper is
...
describe('useSyncplayClient — pre-metadata deferral (#240)', () => {
  ...
  it('does not announce a pause when a buffering element takes a playing state', async () => {
    ...
    // `src/renderer/src/composables/use-syncplay-client.ts:960`, *above* the
    // `clearPendingUserPause()` at
    // `src/renderer/src/composables/use-syncplay-client.ts:996`, so the element
