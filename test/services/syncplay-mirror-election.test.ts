...
describe('SyncplayClient — the room speaking back through our own mirror (#277)', () => {
  ...
  it.each([4000, 4001, 4500, 5000, 5500, 6000, 6500, 7000])(
    ...
    (METADATA_MS) => {
      ...
      // this file rather than five — the sixth is
      // (test/services/syncplay-mirror-election.test.ts:246 ("expect(positions[i]).toBeGreaterThan(positions[i - 1])")),
      // `expected 602.9999999523163 to be greater than 603`. The five even-tick
      // cells red *here* only on the join-time State route; when they do, read
      // the helper before looking for a convergence change in src/. The repair
      // is the same on both routes: drop this term and predict the bare 2/3.
